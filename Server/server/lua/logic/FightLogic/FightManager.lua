...
function FightManager:addStartBuff(attack_buff_list,hurter_buff_list)
	if attack_buff_list ~= nil then
		for i = 1 , #attack_buff_list do
			for row=1,rowMax do
				for col=1,colMax do
					local temp = self.group_[1][row][col]
					if temp and temp:isCanHurt() then
						local weaponConf = CONF.WEAPON.get(attack_buff_list[i])
						if weaponConf then
							temp:calCreateBuff(weaponConf, CONF.EBuffCondition.kNow)
						end
					end
				end
			end
	...
	if hurter_buff_list ~= nil then
		for i = 1 , #hurter_buff_list do
			for row=1,rowMax do
				for col=1,colMax do
					local temp = self.group_[2][row][col]
					if temp and temp:isCanHurt() then
						local weaponConf = CONF.WEAPON.get(hurter_buff_list[i])
						if weaponConf then
							temp:calCreateBuff(weaponConf, CONF.EBuffCondition.kNow)
						end
					end
