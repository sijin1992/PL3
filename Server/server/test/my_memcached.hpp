...
//memcached_success 替换成MY_MEMCACHED_SUCCESS
 #define MY_MEMCACHED_SUCCESS(statement) memcached_success(m_lastret=statement)
...
public:
  ...
  CMyMemcache(const std::string &config)
  {
    memc= memcached(config.c_str(), config.size());
  }

  CMyMemcache(const std::string &hostname, in_port_t port)
  {
    memc= memcached("", 0);
    if (memc)
  ...
  CMyMemcache(memcached_st *clone)
  {
    memc= memcached_clone(NULL, clone);
  }

  CMyMemcache(const CMyMemcache &rhs)
  {
    memc= memcached_clone(NULL, rhs.getImpl());
  }

  CMyMemcache &operator=(const CMyMemcache &rhs)
  {
    if (this != &rhs)
    {
      memcached_free(memc);
  ...
  {
    return (MY_MEMCACHED_SUCCESS(memcached_behavior_set(memc, flag, data)));
  }
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_server_add(memc, server_name.c_str(), port));
  }
  ...
  bool mget(std::vector<std::string> &keys)
  {
    ...
    {
      return MY_MEMCACHED_SUCCESS(memcached_mget(memc, &real_keys[0], &key_len[0], real_keys.size()));
    }
           ...
           uint32_t flags)
  {
    memcached_return_t rc= memcached_set(memc,
                                         ...
                                         &value[0], value.size(),
                                         expiration, flags);
    return MY_MEMCACHED_SUCCESS(rc);
  }
  ...
  bool setByKey(const std::string &master_key,
                ...
                uint32_t flags)
  {
    return MY_MEMCACHED_SUCCESS(memcached_set_by_key(memc, master_key.c_str(),
                                                  master_key.length(),
                                                  key.c_str(), key.length(),
                                                  &value[0], value.size(),
                                                  expiration,
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_increment(memc, key.c_str(), key.length(), offset, value));
  }
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_decrement(memc, key.c_str(),
                                                 key.length(),
                                                 offset, value));
  }
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_add(memc, key.c_str(), key.length(),
  ...
  bool addByKey(const std::string &master_key,
                ...
                const std::vector<char> &value)
  {
    return MY_MEMCACHED_SUCCESS(memcached_add_by_key(memc,
                                                  master_key.c_str(),
                                                  master_key.length(),
                                                  key.c_str(),
                                                  key.length(),
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_replace(memc, key.c_str(), key.length(),
                                               &value[0], value.size(),
                                               0, 0));
  }
  ...
  bool replaceByKey(const std::string &master_key,
                    ...
                    const std::vector<char> &value)
  {
    return MY_MEMCACHED_SUCCESS(memcached_replace_by_key(memc,
                                                      master_key.c_str(),
                                                      master_key.length(),
                                                      key.c_str(),
                                                      key.length(),
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_prepend(memc, key.c_str(), key.length(),
                                               &value[0], value.size(), 0, 0));
  }
  ...
  bool prependByKey(const std::string &master_key,
                    ...
                    const std::vector<char> &value)
  {
    return MY_MEMCACHED_SUCCESS(memcached_prepend_by_key(memc,
                                                      master_key.c_str(),
                                                      master_key.length(),
                                                      key.c_str(),
                                                      key.length(),
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_append(memc,
                                              key.c_str(),
                                              key.length(),
                                              &value[0],
                                              value.size(),
  ...
  bool appendByKey(const std::string &master_key,
                   ...
                   const std::vector<char> &value)
  {
    return MY_MEMCACHED_SUCCESS(memcached_append_by_key(memc,
                                                     master_key.c_str(),
                                                     master_key.length(),
                                                     key.c_str(),
                                                     key.length(),
  ...
  bool cas(const std::string &key,
           ...
           uint64_t cas_arg)
  {
    return MY_MEMCACHED_SUCCESS(memcached_cas(memc, key.c_str(), key.length(),
                                           &value[0], value.size(),
                                           0, 0, cas_arg));
  }
  ...
  bool casByKey(const std::string &master_key,
                ...
                uint64_t cas_arg)
  {
    return MY_MEMCACHED_SUCCESS(memcached_cas_by_key(memc,
                                                  master_key.c_str(),
                                                  master_key.length(),
                                                  key.c_str(),
                                                  key.length(),
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_delete(memc, key.c_str(), key.length(), 0));
  }
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_delete(memc,
                                              key.c_str(),
                                              key.length(),
                                              expiration));
  }
  ...
  bool removeByKey(const std::string &master_key,
                   const std::string &key)
  {
    return MY_MEMCACHED_SUCCESS(memcached_delete_by_key(memc,
                                                     master_key.c_str(),
                                                     master_key.length(),
                                                     key.c_str(),
                                                     key.length(),
  ...
  {
    return MY_MEMCACHED_SUCCESS(memcached_flush(memc, expiration));
  }
  ...
  //add funct
  inline std::string lastretstr()
  {
  	return getError(m_lastret);
  }

  inline int lastretcode()
  {
  	return m_lastret;
  }
