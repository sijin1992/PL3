...
sub do_print_table
{
	...
	$this->write_table_body();
	$this->write_table_foot();
}
...
sub write_table_header
{
	...
	local $title = $this->{'mTitle'};
	local $datetime = `date`;
	&write_format("FORMAT_TABLE_HEADER");
}

sub write_table_foot
{
	my $this = shift;
	&write_format("FORMAT_TABLE_FOOT");
}
...
format FORMAT_TABLE_HEADER =
				<html>
				<body>
				 <table width = "800" border = "1" frame = "box" align = "center">
				 <caption>
				 <big><b>@*</b><big><br/><small>@*</small>
				  $title,$datetime
				 </caption>  
.
