...
#define LOG_STAT_REGIST(uid, ip, mmc, acc, nick, lv, qd) \
	LOG_STAT_DATA(uid, STAT_REGIST, "%s|%s|%s|%s|%d|%d", ip, mmc, acc, nick, lv, qd);

//登录(用户ID，IP地址， MCC移动设备国家码，等级，账号，渠道)
#define LOG_STAT_LOGIN(uid, ip, mmc, level, acc, qd) \
	LOG_STAT_DATA(uid, STAT_LOGIN, "%s|%s|%d|%s|%d", ip, mmc, level, acc, qd);

//充值(用户ID，充值金额，支付方式，是否首充，vip等级，vip积分，渠道)
#define LOG_STAT_DEPOSIT(uid, amount, paytype, isfirst, viplevel, vipscore, qd) \
	LOG_STAT_DATA(uid, STAT_DEPOSIT, "%d|%d|%d|%d|%d|%d", amount, paytype, isfirst, viplevel, vipscore, qd);
...
#define LOG_STAT_USER(uid, nick, qd, lv, exp, viplv, vipscore, totaldep, totalrmb, gold, money, php, maxpower, state, stagelv, maxrank, menpai) \
	LOG_STAT_DATA(uid, STAT_USER, "%s|%s|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%d|%s", nick, qd, lv, exp, viplv, vipscore, totaldep, totalrmb, gold, money, php, maxpower, state, stagelv, maxrank, menpai);

//背包快照(用户ID，凯旋令，龙纹令，武魂，门派威望，门派奖章)
#define LOG_STAT_BAG(uid, kxl, lwl, wh, mpww, mpjz) \
	LOG_STAT_DATA(uid, STAT_BAG, "%d|%d|%d|%d|%d", kxl, lwl, wh, mpww, mpjz);


//门派快照(门派ID，名称，等级，库银，绝学，创建时间)
#define LOG_STAT_MENPAI(mpid, name, level, gold, jx, ct) \
	LOG_STAT_DATA(mpid, STAT_MENPAI, "%s|%d|%d|%d|%d", name, level, gold, jx, ct);
