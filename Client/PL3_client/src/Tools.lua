...
function Tools.getEnergyAddition( ship_id, energy_level, attr_type)
	...
	if not conf or energy_level == nil or energy_level < 1 then
		return 0
	end
...
function Tools.calShipFightPower(ship_info, weapon_id_list)
	...
	if Tools.isEmpty(weapon_id_list) == false then
		for i,v in ipairs(weapon_id_list) do
			if v ~= 0 then
				local conf = CONF.WEAPON.check(v)
				if conf then 
...
function Tools.getValueByTechnologyAddition( value, tech_1, tech_2, tech_3, tech_list, group_tech_list)
	...
	--print("getValueByTechnologyAddition",value,percent,num,sign,math.ceil( value * 0.01 * percent * sign + num * sign ) )
...
--			dump(user_info.achievement_data)
...
function Tools.getRewards( reward_id )
	local conf = CONF.REWARD.get(reward_id)
	if conf == nil then
		print("CONF.REWARD.get() err", reward_id)
		LOG_ERROR(string.format("CONF.REWARD.get() err", reward_id))
	end
