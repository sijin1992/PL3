...
local EnemyLayer = class("EnemyLayer", cc.load("mvc").ViewBase)

EnemyLayer.RESOURCE_FILENAME = "ColonizeScene/EnemyLayer.csb"

EnemyLayer.RUN_TIMELINE = true

EnemyLayer.NEED_ADJUST_POSITION = true

EnemyLayer.RESOURCE_BINDING = {
	["close"]={["varname"]="",["events"]={{["event"]="touch",["method"]="OnBtnClick"}}},
}

function EnemyLayer:OnBtnClick(event)
...
function EnemyLayer:onCreate(data)
	self.data_ = data
end

function EnemyLayer:onEnter()
	
	printInfo("EnemyLayer:onEnter()")
end

function EnemyLayer:onExit()
	
	printInfo("EnemyLayer:onExit()")

end

function EnemyLayer:resetStars( ... )
	...
	for i,v in ipairs(self.slave_brief_info_list) do
		...
		star:getChildByName("zb_line_top_2"):setTexture("Common/newUI/zb_line_top_red.png")
		...
		star:getChildByName("touch"):addClickEventListener(function ( ... )
			...
			-- self:getApp():pushView("newFormScene", {from = "slave", type = 1, user_name = v.user_name, name = v.nickname, icon_id = v.icon_id})
...
function EnemyLayer:createInfoNode( info )
	...
	node:getChildByName("enslave_button"):addClickEventListener(function ( ... )
		...
		if info.master == nil or info.master == "" then
			self:getApp():addView2Top("NewFormLayer", {from = "slave", type = 1, user_name = info.user_name, name = info.nickname, icon_id = info.icon_id, layer = "enemy"})
		else
			self:getApp():addView2Top("NewFormLayer", {from = "slave", type = 1, user_name = info.user_name, name = info.master_nickname, icon_id = info.icon_id, layer = "enemy"})
		end
	end)
...
function EnemyLayer:onEnterTransitionFinish()
	printInfo("EnemyLayer:onEnterTransitionFinish()")
	...
	rn:getChildByName("my_slave"):setString(CONF:getStringValue("settlement_record"))
	rn:getChildByName("my_slave_ins"):setString(CONF:getStringValue("slave_describe"))

	-- rn:getChildByName("guize"):getChildByName("text"):setString(CONF:getStringValue("rule"))

	rn:getChildByName("zhua_text"):setString(CONF:getStringValue("arrest_slave"))
	...
	local function click_Callback( sender, eventType )
		...
		elseif eventType == ccui.TouchEventType.ended then 
			...
			if sender:getName() == "zhua" then
				self:getParent():changeLayer("slave")
	...
	rn:getChildByName("zhua"):addTouchEventListener(click_Callback)
	...
	local function recvMsg()
		...
		if cmd == Tools.enum_id("CMD_DEFINE","CMD_SLAVE_SEARCH_RESP") then
			...
			if proto.result == "EMPTY" then
				...
				rn:getChildByName("empty"):getChildByName("konque_line_4"):setPositionX(-(rn:getChildByName("empty"):getChildByName("text"):getContentSize().width + 22)/2)
		...
		elseif cmd == Tools.enum_id("CMD_DEFINE","CMD_GET_CHAT_LOG_RESP") then
            ...
            if proto.result < 0 then
                ...
                if #proto.log_list > 0 then
	                -- local info = proto.log_list[#proto.log_list]
	                local time = 0
					local str = ""
					local tt 
    ...
    require("util.ExConfigScreenAdapter"):getInstance():onFixQuanmianping_EnemySlave(self)

end



function EnemyLayer:onExitTransitionStart()
	printInfo("EnemyLayer:onExitTransitionStart()")
