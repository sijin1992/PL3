...
local PlanetNode = class("PlanetNode", cc.load("mvc").ViewBase)
...
PlanetNode.RESOURCE_FILENAME = "ChatLayer/world_list.csb"

function PlanetNode:onEnterTransitionFinish()

end

function PlanetNode:init(scene,data)
    ...
    self.planetListener_ = cc.EventListenerCustom:create("planetMsg", function (event)
    ...
    eventDispatcher:addEventListenerWithFixedPriority(self.planetListener_, FixedPriority.kNormal)

end

function PlanetNode:resetList( ... )
	local rn = self:getResourceNode()
    ...
    local function createListItem( info )
    	...
    	string_2:setPositionX(0)
    	string_3:setPositionX(string_2:getContentSize().width + 2)
        ...
        if info.nickname == player:getNickName() then
            ...
            string_2:setPositionX(string_3:getPositionX() - 2 - string_2:getContentSize().width)
        ...
        btn:setContentSize(cc.size(string_3:getContentSize().width + string_2:getContentSize().width, string_3:getContentSize().height + 3))
...
function PlanetNode:onExitTransitionStart()

    printInfo("PlanetNode:onExitTransitionStart()")

    local eventDispatcher = self:getEventDispatcher()
    eventDispatcher:removeEventListener(self.recvlistener_)
    eventDispatcher:removeEventListener(self.planetListener_)
